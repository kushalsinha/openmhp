...
function main() {
  ...
  switch (cmd) {
    ...
    case "status": ensureRuntime({ upgrade: true }); process.exit(run("mhp", ["node", "--service-status"]));
    case "stop": ensureRuntime({ upgrade: true }); process.exit(run("mhp", ["node", "--uninstall-service"]));

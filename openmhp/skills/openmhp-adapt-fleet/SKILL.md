...
On the scientist's machine, `mhp lab add http://bay3-pc:18921` (or the `mhp_lab` tool) for each
served device; `mhp lab scan bay3-pc bay1-pc` finds them if they advertise or answer `/mhp.json`.
For labs with hundreds of devices, build a shared directory instead:

...
class MadsciPaths:
    ...
    action_cancel = "/action/{id}/cancel"


_TERMINAL = {"succeeded": "done", "failed": "failed", "cancelled": "cancelled"}


def madsci_node(url: str, *, device: dict, http=None, paths: type = MadsciPaths, poll_s: float = 0.5,
                approval: dict | None = None, physical: dict | None = None, cancel_timeout_s: float = 30.0) -> BoundDriver:
    ...
    def action(name, meta):
        def run(job, params):
            res = http.post(paths.action, {"action_name": name, "args": params}) or {}
            aid = res.get("action_id") or res.get("id")
            status = res.get("status")
            cancel_sent_at = None
            while status not in _TERMINAL:
                if not aid:
                    raise RuntimeError(f"MADSci returned no action id and no terminal status ({status!r}); outcome unknown")
                if job.cancel_requested and cancel_sent_at is None:
                    http.post(paths.action_cancel.format(id=aid))          # ask the backend to stop, then keep watching
                    cancel_sent_at = time.monotonic()
                if cancel_sent_at is not None and time.monotonic() - cancel_sent_at > cancel_timeout_s:
                    raise RuntimeError(f"cancel requested but the node still reports {status!r} after {cancel_timeout_s}s; "
                                       "physical outcome unknown")
                time.sleep(poll_s)
                res = http.get(paths.action_result.format(id=aid)) or {}
                status = res.get("status")
            if status == "failed":

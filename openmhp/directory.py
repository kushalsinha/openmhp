...
class Directory:
    ...
    def __init__(self, ping_timeout: float = PING_TIMEOUT_S, state_ttl: float = STATE_TTL_S):
        ...
        self._inflight: dict[str, Any] = {}              # id -> Future of an in-progress remote ping
    ...
    def remove(self, cid: str) -> bool:
        """Forget a device entirely: card, target, cached state and ping client."""
        if cid not in self.cards:
            return False
        for t in self._tf.get(cid, {}):
            self._df[t] -= 1
        for table in (self.cards, self.targets, self._probes, self._state, self._tf, self._len, self._clients, self._inflight):
            table.pop(cid, None)
        self.updated = time.time()
        return True
    ...
    def _ping(self, cid: str) -> str:
        ...
        if dev is None:
            dev = connect(self.targets[cid], cid)
            dev.timeout = max(self.ping_timeout * 2, 0.5)
            self._clients[cid] = dev
        return dev.call("ping")["state"]

    def live_state(self, cid: str) -> str:
        return self.refresh([cid])[cid]

    def refresh(self, ids: list[str]) -> dict[str, str]:
        """State for many devices. Cached answers younger than state_ttl are reused; in-process probes run
        inline; remote pings run in the pool (one in flight per device) and are collected against one
        deadline, so a saturated pool cannot deadlock on itself."""
        now, out, pending = time.time(), {}, {}
        for cid in ids:
            cached = self._state.get(cid)
            if cached and now - cached[1] < self.state_ttl:
                out[cid] = cached[0]
                continue
            if cid in self._probes:
                try:
                    state = self._probes[cid]()
            ...
            with self._inflight_lock:
                ...
                if fut is None or fut.done():
                    fut = self._inflight[cid] = self._pool.submit(self._ping, cid)
            pending[cid] = fut
        if pending:
            wait(list(pending.values()), timeout=self.ping_timeout)
